...
export default function CryptoListings({ children }) {
  ...
  useEffect(() => {
    setCurrentPosts([...listingArray.slice(indexOfFirstPost, indexOfLastPost)]);
  }, [alreadySortedByASC]);
  ...
  return (
    <ListingsBox>
      ...
      {indexOfLastPost === listingArray.length ? (
        <div className="end-of-listing">-- End Of Listing --</div>
      ) : (
        <button
          onClick={loadMore}
          disabled={indexOfLastPost === listingArray.length ? true : false}
        >
          View More
        </button>
      )}
